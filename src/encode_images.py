...
class EncodeImageScreen(QWidget):
    def __init__(self, switch_to_steganography):
        ...
        self.result_label = QLabel("Encoded Image will be shown here", self)
    ...
    def cover_dropEvent(self, event):
        ...
        if urls and urls[0].isLocalFile():
            self.cover_image_path = urls[0].toLocalFile()
            pixmap = QPixmap(self.cover_image_path)
            if not pixmap.isNull():
                max_width = self.cover_label.width()
                max_height = self.cover_label.height()
    ...
    def encode_message(self):
        if not self.cover_image_path:
            self.result_label.setText("No cover image selected")
            return
        ...
        if not self.payload_data:
            self.result_label.setText("No payload data provided")
            return

        lsb_use = int(self.bits_dropdown.currentText())
        try:
            encoded_image = stego_image.encode(self.cover_image_path, self.payload_data, lsb_use)
            encoded_image_path = os.path.splitext(self.cover_image_path)[0] + "_encoded.png"
            cv2.imwrite(encoded_image_path, encoded_image)
            self.result_label.setText(f"Encoded image saved as: {encoded_image_path}")

            pixmap = QPixmap(encoded_image_path)
            self.result_label.setPixmap(
                pixmap.scaled(self.result_label.width(), self.result_label.height(), Qt.KeepAspectRatio,
    ...
    def backbutton_clear(self):
        ...
        self.payload_label.setText("Drag and drop your payload (text file) here or type your message below")
        self.text_edit.setPlaceholderText("Or type your secret message here")
        self.cover_label.setText("Drag and drop your cover image here")
        self.result_label.setText("Encoded Image will be shown here")
